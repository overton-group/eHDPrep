...
The simple methods above impute each variable independently. `impute_missing_values()` also supports **k-nearest-neighbours (kNN) imputation** (`method = "knn"`), which estimates each missing value from the `k` most similar rows (samples), using the remaining variables as predictors. Similarity is measured with Gower distance, allowing mixed numeric and categorical data to be handled together. Because it draws on the whole row's profile rather than a single column-wide summary, kNN better preserves the relationships between variables.

kNN imputation is provided by the 'VIM' package, which is only required (a suggested, not mandatory, dependency) when this method is used. Variables which should not be used to compute similarity (such as a row identifier) can be excluded with the `ignore` argument:
