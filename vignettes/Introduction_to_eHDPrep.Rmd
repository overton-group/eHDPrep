...
The data types for `example_data`, after the manual amendments described above, are shown below. `assume_var_classes()` can only infer a class from the values it sees, so the domain-specific classes are supplied by the user at this stage: `patient_id` becomes `"id"`, `t_stage` and `n_stage` become the precoded ordinal types, the two SNP variables become `"genotype"`, and `free_text` becomes `"freetext"`. The two-category variables `hypertension` and `rural_urban` are detected as `"binary"` automatically and are kept as such.

`diabetes_type` illustrates why this manual review step matters. It has exactly two non-missing values (`"Type I"` and `"Type II"`), so it is detected as `"binary"`, but clinically it is an ordinal classification. It is therefore amended to `"ordinal"`, and because its levels are not precoded it is left unmodified by `apply_quality_ctrl()`. Automatic detection describes the values; only the user can supply the meaning.
...
-   `bin_cats:` A character vector showing how variables with two options should be encoded with the syntax `negative_finding = positive_finding`. If positivity/negativity is not associated with the binary categories of a variable (e.g. `rural_urban` in `example_data`) then the ordering can be arbitrarily decided. A pair must be supplied for every variable classed as `"binary"`; a binary variable whose two values match no pair in `bin_cats` is left unmodified.
