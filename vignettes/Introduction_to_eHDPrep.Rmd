...
author: "Tom Toner, Kluivert Boakye Duah, Ian Overton"
output:
  pdf_document:
    toc: yes
    toc_depth: 3
...
The permitted datatypes are: `"id"`, `"numeric"`, `"double"`, `"integer"`, `"character"`, `"factor"`, `"binary"`, `"ordinal"`, `"ordinal_tstage"`, `"ordinal_nstage"`, `"genotype"`, `"freetext"`, `"logical"`, and `"date"`. Note that `ordinal` variables are not modified by `apply_quality_ctrl()` as the ordinal classes would need to be specified for each variable. 'eHDPrep' provides two special ordinal data types `"ordinal_tstage"` and `"ordinal_nstage"` for two common cancer staging measures where the orders are precoded.

`assume_var_classes()` labels character and factor variables with exactly two unique non-missing values as `"binary"`, and those with more than two but no more than `factor_threshold` (default `5`) unique values as `"factor"`. Factor detection can be disabled with `factor_threshold = 0` or `NULL`. Variables classed as `"numeric"` are converted with `coerce_numeric_vars()`, which reports any values that could not be parsed and were therefore set to `NA`; this is applied automatically by `apply_quality_ctrl()`.

Date and date-time variables are labelled `"date"`. Dates frequently arrive as text, so variables classed as `"date"` are converted with `coerce_dates()`, which parses common formats (`2020-01-15`, `15/01/2020`, and similar) and reports any values it could not interpret:

```{r}
coerce_dates(tibble::tibble(date_of_diagnosis = c("2020-01-15", "2020-03-15",
                                                  "not recorded")),
             date_of_diagnosis)
```

Day-first formats are recognised too, so `"15/01/2020"` is read as 15 January 2020 rather than the year 15. A single format is chosen per variable, however: this keeps the interpretation of ambiguous dates consistent across a variable, but means that a variable mixing formats will have the values in the minority format reported and set to `NA` rather than silently parsed under a different interpretation. Where this occurs, standardise the variable before quality control.

Beyond this coercion, date variables are left unmodified by `apply_quality_ctrl()`, as they are already ordered and machine-interpretable. They participate in the other quality control steps as any other variable: they are included in completeness assessment, can be imputed (with the median date, since dates are ordered), can be compared in [Internal consistency] rules such as requiring a date of birth to precede a date of death, and are converted to days since 1970-01-01 by `encode_as_num_mat()`.

The data types for `example_data`, after the manual amendments described above, are shown below. `assume_var_classes()` can only infer a class from the values it sees, so the domain-specific classes are supplied by the user at this stage: `patient_id` becomes `"id"`, `t_stage` and `n_stage` become the precoded ordinal types, the two SNP variables become `"genotype"`, and `free_text` becomes `"freetext"`. The two-category variables `hypertension`, `rural_urban` and `diabetes_type` are detected as `"binary"` automatically and are kept as such. In the case of `diabetes_type`, `"Type I"` and `"Type II"` are distinct classifications rather than ordered levels, so an unordered two-level encoding is appropriate; a variable whose two categories *were* ordered would instead be classed as `"ordinal"` and given explicit levels.

Note also that `diabetes` is detected as `"factor"` rather than `"binary"` because it has three distinct raw values (`"Yes"`, `"No"` and the missingness string `"missing"`). Once missingness is standardised to `NA` (see [Encoding missing values]) only two values remain, and `apply_quality_ctrl()` treats it as binary from that point on.
...
| `ordinal`             | Ignored                                                      |
| `logical`             | Ignored                                                      |
| `date`                | Coerced to `Date` with `coerce_dates()` if not already a date; values which cannot be parsed are set to `NA` and reported. Otherwise left unmodified, as dates are already ordered and machine-interpretable |
...
| `binary`              | Converted to ordered factor with two levels, using the pair given in the `bin_cats` parameter (see `?encode_binary_cats`). Mixed boolean encodings (e.g. `"no"`, `"N"`, `"False"`) are normalised before matching. If no pair in `bin_cats` matches the variable's two values, it is left unmodified |
...
-   `bin_cats:` A character vector showing how variables with two options should be encoded with the syntax `negative_finding = positive_finding`. If positivity/negativity is not associated with the binary categories of a variable (e.g. `rural_urban` in `example_data`) then the ordering can be arbitrarily decided. A pair must be supplied for every variable classed as `"binary"`; a binary variable whose two values match no pair in `bin_cats` is left unmodified. Here `"Type I" = "Type II"` is included so that `diabetes_type` is encoded — the ordering carries no meaning in this case, as the two diabetes types are distinct classifications rather than ranked levels.
...
-   `impute`: Optionally, missing values can be imputed by setting `impute = TRUE` (default `FALSE`). The strategy is chosen with `impute_method` (default `"auto"`; see [Imputing missing values](#impute-missing-values)). Imputation is applied after missing values are standardised but before any encoding, so that it operates on the raw variable values. All variables except `id_var` and free-text variables are imputed.
...
5.  Constraining a single variable against fixed values (e.g. `age` must be `>= 70`, or must lie within `0:120`)
...
#### Constraining a single variable

Not every rule involves two variables. A variable may also be checked against fixed values, which is useful for range and plausibility checks. Such a rule leaves the second column `NA` and gives the boundary in the fourth column. Two forms are available:

-   **Comparison against a value.** The third column holds the logical test and the fourth the value to compare against. For example `"tumoursize", NA, ">=", "0", NA` requires every tumour size to be non-negative.
-   **Permitted values.** The third column is `NA` and the fourth holds either a colon-separated numeric range or a single permitted category. For example `"tumoursize", NA, NA, "0:100", NA` requires every tumour size to lie between 0 and 100.

Applying a plausibility check to `tumoursize` reveals that `example_data` contains negative measurements:

```{r}
plausibility_rules <- tibble::tribble(~varA, ~varB, ~lgl_test, ~varA_boundaries, ~varB_boundaries,
                                      "tumoursize", NA, ">=", "0", NA
                                      )

identify_inconsistency(data = example_data, consis_tbl = plausibility_rules,
                       id_var = "patient_id")
```

Single-variable and two-variable rules can be combined in the same `consis_tbl`. Note that missing (`NA`) values are never reported by a single-variable rule, as they cannot be assessed against a boundary; use `assess_completeness()` to review missingness instead (see [Measure completeness]).
...
### Imputing missing values {#impute-missing-values}

Once missingness is consistently represented as `NA`, remaining missing values can be *imputed* (filled in with estimated values) using `impute_missing_values()`. Imputation is useful for downstream analyses but it should be applied thoughtfully as it introduces estimated, rather than observed values. The single-value methods below assume values are missing completely at random; where this does not hold, imputation can bias subsequent analyses, and the mechanism of missingness should be considered first [@little2019].

The `method` argument selects the imputation strategy:

| `method`     | Behaviour                                                                                             |
| ------------ | ----------------------------------------------------------------------------------------------------- |
| `"auto"`     | *(default)* numeric variables imputed with their median; non-numeric variables imputed with their mode (most frequent value) |
| `"median"`   | numeric variables imputed with their median (non-numeric fall back to mode)                           |
| `"mean"`     | numeric variables imputed with their mean (non-numeric fall back to mode)                             |
| `"mode"`     | all selected variables imputed with their mode                                                        |
| `"constant"` | missing values replaced with a fixed value supplied via `constant`                                    |
| `"knn"`      | k-nearest-neighbours imputation (see below)                                                           |

By default all variables are imputed; specific variables can be selected by passing them (unquoted) as additional arguments. The number of values imputed per variable is reported as a message, so imputation can be reviewed alongside other quality control steps with `review_quality_ctrl()`.

```{r}
# median (numeric) / mode (categorical) imputation of all variables:
impute_missing_values(example_data)

# impute only diabetes_type, using the mode:
impute_missing_values(example_data, diabetes_type, method = "mode")
```

The simple methods above impute each variable independently. `impute_missing_values()` also supports **k-nearest-neighbours (kNN) imputation** (`method = "knn"`), which estimates each missing value from the `k` most similar rows (samples), using the remaining variables as predictors. Similarity is measured with Gower distance [@gower1971], allowing mixed numeric and categorical data to be handled together. Because it draws on the whole row's profile rather than a single column-wide summary, kNN better preserves the relationships between variables.

kNN imputation is provided by the 'VIM' package [@kowarik2016], which is only required (a suggested, not mandatory, dependency) when this method is used. Variables which should not be used to compute similarity (such as a row identifier) can be excluded with the `ignore` argument:

```{r, eval = requireNamespace("VIM", quietly = TRUE)}
# k-nearest-neighbours imputation, excluding the identifier from the
# distance calculation (requires the 'VIM' package):
impute_missing_values(example_data, method = "knn", ignore = patient_id, k = 5)
```

Imputation can also be performed as part of the high-level `apply_quality_ctrl()` workflow by setting `impute = TRUE` (and, optionally, `impute_method`). See [Apply quality control].
